...
```mermaid
flowchart LR
    Goal[LongTaskRunner<br/>Goal and Round] --> Agent[Agent.runTurn<br/>Turn and Step]
    Session[SessionLog<br/>append-only events] --> Request[buildRequest<br/>model request]
    Context[Context<br/>plugins, tools, and prompts] --> Agent
    Context --> Request
    Agent --> Session
    Agent --> Request
    Request --> Provider[model adapter]
    Provider --> Agent
...
| File | Responsibility |
|---|---|
| [`src/protocol.ts`](./src/protocol.ts) | Provider-independent messages, requests, streaming events, and tool contracts |
| [`src/agent.ts`](./src/agent.ts) | Agent Loop, stopping conditions, argument validation, and tool execution |
| [`src/context.ts`](./src/context.ts) | Tool-result projection, request parts, token estimates, and stable-prefix comparison |
| [`src/runtime.ts`](./src/runtime.ts) | Plugin mounting, Services, Tools, Prompts, events, and effect lifecycles |
| [`src/session.ts`](./src/session.ts) | Append-only events, context checkpoints, request reconstruction, and trace generation |
| [`src/runtime-tools.ts`](./src/runtime-tools.ts) | Dynamic plugin inspection, definition, execution, stopping, and removal |
| [`src/long-task.ts`](./src/long-task.ts) | Goal state, Round progression, and stopping reasons |
| [`src/llm-fake.ts`](./src/llm-fake.ts) | Deterministic model responses for offline runs |
| [`src/llm-deepseek.ts`](./src/llm-deepseek.ts) | DeepSeek streaming Chat Completions adapter |
