...
`word_count` 只保留为验证动态定义、运行、使用和移除的最小机制示例，不预先占据贯穿任务的位置。
...
贯穿任务确定为 **“修复购物车重复优惠 Bug”**：离线工作区内有固定的 issue、结账源码、回归测试与持续集成（CI）失败日志。订单级优惠已经分摊进商品小计，却又在 `calculateTotal()` 的返回值中被扣除一次。Agent 要定位重复扣减，编写并运行 `typescript_analysis` Cordis 插件检查调用方与类型，验证后移除插件，用精确补丁修复源码，运行测试，并提交唯一正确、可由程序验收的 `CHECKOUT-417` 补丁。任务不开放任意文件读取或通用 Shell；所有输入和修改都由受限工作区工具提供。
...
| M05 | 固定组装不能在运行时回答“这个改动影响哪些调用方、类型仍然成立吗”。 | Agent 先检查运行时，再定义并运行 `typescript_analysis` Cordis 插件，调用 `find_references` 与 `check_types`，最后停止并移除插件；相邻请求的 tool schemas、Prompt 和插件图随日志变化。`word_count` 另作最小机制验收。 |
...
DSH 网页端的创造模式具备标准模式的全部能力，并增加运行时检查、动态 Cordis 插件实验和 preset 创作指导；其产品目标是创建自定义 Agent preset。Agent 可以编写动态插件，在当前进程中运行并验证新增能力，再把验证结果用于正式插件或 preset 的创作。

Nano 实现其中最能解释插件内核的部分：把运行时检查、动态插件定义、运行、停止和移除暴露成常驻工具。Agent 编写一项新的 Cordis Plugin，通过 M03 的普通安装路径挂载，下一次请求出现该工具；完成实验后再通过同一个 disposer 卸载，后续请求中的工具目录随之变化。

Session Log 记录插件定义、挂载、调用和移除，网站同步展示工具列表、Prompt 贡献和插件图的变化。`word_count` 用于机制级验收；本章最后说明验证后的插件组装怎样进入自定义 preset。贯穿任务若不自然需要插件实验，就重新评估 M05 的位置而不是扭曲任务。
...
5. M05 的运行时检查、动态插件定义、运行和移除改变下一次请求的 tool schemas，并由同一日志和 Trace 呈现；教程说明插件实验与 preset 创作之间的关系。
6. 若某项增量无法自然出现在同一任务中，先回到第 2 节调整范围。

`word_count` 另有一个小型确定性场景，专门证明“定义前不可见—运行后调用—移除后消失”，但它不替代贯穿任务。
...
- `cordis_inspect`：查看已安装插件、服务、工具、Prompt 贡献和关系。
- `cordis_define`：登记 Agent 编写的 Cordis 插件代码。
- `cordis_run`：把已经定义的插件挂载到当前 Context。
- `cordis_stop`：停止插件并保留定义。
- `cordis_undefine`：停止插件并删除定义。

动态插件通过普通 `ctx.mount()` 安装并通过同一个 disposer 卸载，不为演示增加特殊注册路径。`word_count` 用于展示最小的 define／run／use／undefine 闭环，`typescript_analysis` 用于贯穿任务。
...
实现运行时检查与动态 Cordis 插件工具，完成贯穿任务中的自然插件实验，并用 `word_count` 场景补足机制级边界。验证定义、运行、实际调用和移除前后的 Prompt 与 tool schemas 变化；用结果解释创造模式怎样把经过验证的插件组装写成自定义 preset。
...
- M05 同步展示动态插件定义、运行、调用和移除时的工具列表与插件图，并说明该实验怎样服务于创造模式的 preset 创作目标。
