...
M01–M05 是首版入门主线候选，M06 是后移候选。最终范围由贯穿任务筛选，而不是由候选功能数量决定。关注问题与筛选依据见 [Nano DSH 内容候选](nano-dsh-curriculum/NANO-DSH-CANDIDATES.md)。

## 2. 内容筛选与范围决策

### 2.1 贯穿任务

M01–M05 使用同一个任务展示机制逐步加入后的差异。任务尚未确定；编码前必须先选定，并同时满足：
...
若贯穿任务证明需要多轮续行，再实现最小长程机制：保存当前目标、状态和已开始的 Round 数；每轮结束后判断是否继续；完成、阻塞或达到轮次上限时停止。fake 场景必须确定性展示多个 Round 推进同一目标。
...
主线逻辑是：**先看标准模式采用的普通 Agent Loop → 对照 PTC 的工具呈现方式 → 组织模型上下文 → 解释不同 preset 如何由插件组装 → 用日志重建全部经历 → 通过运行时检查与插件实验理解创造模式**。只有任务确实需要跨 Round 续行时，才在末尾加入 M06。
...
只有范围清单明确选择后，才实现上下文溢出恢复或 M06。M06 独立形成 `tutorial-m06` checkpoint；未选择时不保留半成品。

### G. 教程与网站

每完成 M01–M05 的代码、测试和确定性场景，就编写对应章节并建立 `tutorial-m01` 至 `tutorial-m05` tag。每章包含：本章问题、前一步的具体不足、新增或修改的代码、运行结果、请求或 Trace 的变化，以及生产版 DSH 额外处理的内容。
...
- 首页展示 M01–M05 的递进关系；只有选择 M06 时才显示第六章。
